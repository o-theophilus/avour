 ...
 * Mon Sep 20 2021 09:23:55 GMT+0100 (West Africa Standard Time)
...
var maxIntDist, video, picked_object, action_02, x1, action_01, pickedObj, i, time, x2, point, goto, pickedObj_name, screen_active, dist, dura, element_id, lookat;
...
function click() {
  if (action_01 == 'action.walk') {
    ...
    if (!!point.length) {
      ...
      setObjTransform('foot', 'position', getVectorValue(goto, 'X'), getVectorValue(goto, 'Y'), getVectorValue(goto, 'Z'), false);
      setObjTransform('head', 'rotation', getObjTransform(getActiveCamera(), 'rotation', 'x'), getObjTransform(getActiveCamera(), 'rotation', 'y'), getObjTransform(getActiveCamera(), 'rotation', 'z'), false);
      tweenCamera('head', 'direction', 3, function() {}, 1);
    }
  } else if (action_01 == 'action.goto') {
    goto = getObjTransform(dictGet(getObjCustomProps(picked_object), 'goto'), 'position', 'xyz');
    setObjTransform('foot', 'position', getVectorValue(goto, 'X'), getVectorValue(goto, 'Y'), getVectorValue(goto, 'Z'), false);
...
function create_mat_swatch() {
  ...
  for (var count = 0; count < 10; count++) {
    i = i + 1;
    element_id = 'mat_' + String(i);
    removeHTMLElement(element_id, true);
  }
  if (dictGet(getObjCustomProps(_pGlob.pickedObject), 'mat')) {
    ...
    for (var count2 = 0; count2 < repeat_end; count2++) {
      i = i + 1;
      element_id = 'mat_' + String(i);
      addHTMLElement('div', element_id, 'TO', 'mat_block', true);
      setHTMLElemAttribute('className', 'mat', element_id, true);
    }
  }
}
...
registerOnHover([['GROUP', 'items'], ['GROUP', 'item_place']], false, function() {
  setHTMLElemStyle('cursor', 'pointer', ['CONTAINER'], false);
}, function() {
  setHTMLElemStyle('cursor', 'default', ['CONTAINER'], false);
});

registerOnHover('walkable', false, function() {
...
registerOnDrag(['GROUP', 'items'], false, [0,1,2], function() {}, function() {
...
eventHTMLElem('click', 'panel_static_goto_details', true, function(event) {
  ...
  setObjTransform(pickedObj, 'position', getObjTransform('profile.point', 'position', 'x'), getObjTransform('profile.point', 'position', 'y'), getObjTransform('profile.point', 'position', 'z'), false);
  setObjTransform('profile.cam', 'position', getObjTransform('profile.point', 'position', 'x'), getObjTransform('profile.point', 'position', 'y') + 1, getObjTransform('profile.point', 'position', 'z'), false);
...
createCSSRule('.mat', ['width: 50px;','height: 50px;','background-color: #94d915;','border: 3px solid black;','border-radius: 50%;','pointer-events: auto;','cursor: pointer;'].join(''), true);

eventHTMLElem('click', 'mat_01', true, function(event) {
  assignMat(pickedObj, String(pickedObj_name) + '.01');
});
eventHTMLElem('click', 'mat_02', true, function(event) {
  assignMat(pickedObj, String(pickedObj_name) + '.02');
});
eventHTMLElem('click', 'mat_03', true, function(event) {
  assignMat(pickedObj, String(pickedObj_name) + '.03');
});
eventHTMLElem('click', 'mat_04', true, function(event) {
  assignMat(pickedObj, String(pickedObj_name) + '.04');
});
eventHTMLElem('click', 'mat_05', true, function(event) {
  assignMat(pickedObj, String(pickedObj_name) + '.05');
});
